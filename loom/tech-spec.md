...
This specification outlines the data flow and API interactions between the Lending liquidation engine (**Atomliq**), the Yield tokenization primitive (**YieldSplitter**), and the B2D infrastructure layer (**Loom** - SolFabric).

### 1. The Loom (SolFabric) Execution Pipeline

**Loom** (SolFabric) provides the underlying connectivity and execution assurance. It abstracts the complexity of Jito Block Engines and custom RPC nodes into a unified developer interface.
...
**Atomliq** leverages Loom (SolFabric) to ensure that liquidations are never front-run and always execute at the intended price.

#### **The Execution Loop:**

1. **Monitor:** Atomliq uses Loom's gRPC stream to monitor account health in real-time.
...
-   **Tx 3:** Jito Tip (to Loom's tip account).

4. **Execute:** The bundle is sent via `Loom.sendBundle()`, guaranteeing the liquidation happens exactly at the price pulled in Tx 1.

### 3. YieldSplitter: Yield Stripping & AMM Management

**YieldSplitter** uses Loom (SolFabric) to maintain its high-frequency SY-PT (Standard Yield to Principal Token) AMM.

-   **Dynamic Curve Adjustment:** As SOL staking rates or MEV rewards fluctuate, YieldSplitter must adjust its AMM bonding curves.
-   **Atomic Swaps:** Traders can swap between SOL and PT/YT. Loom ensures these multi-hop swaps are atomic, preventing the "partial fill" risk common in legacy cross-program invocations (CPI).
...
| Feature            | Atomliq (Lending)           | YieldSplitter (Derivatives) | Loom (Infrastructure)     |
| ------------------ | --------------------------- | --------------------------- | ------------------------- |
| **Data Source**    | Pyth Pull Oracle            | JitoSOL / LST Staking Rates | gRPC ShredStream          |
| **Execution Tool** | Jito Bundles (via Loom)     | Parallel Swap Coordinator   | Custom RPC / Block Engine |
...
1. **Phase 1 (Loom Alpha):** Launch dedicated RPCs with ShredStream access for HFT teams.
2. **Phase 2 (Atomliq Launch):** Integrate Loom with lending protocols to replace legacy "push" liquidation bots.
